...
def events_to_voxel_grid(events, num_bins, height, width, normalize=True):
    ...
    voxel_grid = np.zeros((num_bins, height, width), np.float32).ravel()  # ravel将多维数组变成一维
...
def rectify_events(x: np.ndarray, y: np.ndarray, rectify_map):
    # From distorted to undistorted 从失真到不失真
...
if __name__ == '__main__':
    ...
    parser.add_argument('--event_path', type=str, default='E:/Git/Papers/TMA/DSE/train_events')
    parser.add_argument('--gt_path', type=str, default='E:/Git/Papers/TMA/DSE/train_optical_flow')
    parser.add_argument('--dst', type=str, default='', help='saved path')
    ...
    for scene in scenes:    # e.g. zurich_city_02_a
        ...
        timestamp_f = os.path.join(gt_path, scene, 'flow', 'forward_timestamps.txt')    # 光流时间戳文件
        timestamps = np.genfromtxt(timestamp_f, delimiter=',')  # 光流时间戳
        gt_flow_f = sorted(glob(os.path.join(gt_path, scene, 'flow', 'forward', '*.png')))  # 光流
        ...
        for idx in tqdm(range(len(timestamps)), ncols=60):  # ncols: 调整进度条宽度
            t_curr = timestamps[idx, 0]  # 事件体的起始时间
            t_next = timestamps[idx, 1]  # 事件体的结束时间
            dt = 100 * 1000  # us 100000us(100ms) 相邻segments间的时间跨度
            t_prev = t_curr - dt

            # current event volume
            events_curr = slicer.get_events(t_curr, t_next)  # 获取给定时间窗口内的事件数据
            p = events_curr['p']
            t = events_curr['t']
            x = events_curr['x']
            y = events_curr['y']
            ...
            x_rect = x_rect[mask]   # 屏蔽掉无效的点1305307->1187732
            y_rect = y_rect[mask]   # 1305307->1187732
            p = p[mask] # 1305307->1187732
            t = t[mask] # 1305307->1187732
            events_curr = np.stack([x_rect, y_rect, t, p], axis=1)  # 校正后的事件数据
