...
class MPA(nn.Module):
    ...
    def forward(self, mfs):
        ...
        concat0 = torch.cat(mfs, dim=0).flatten(-2).permute(0, 2, 1)          # [B*T, H*W, C]   [1,2,...,g] MFi
        concat1 = torch.cat(mfs[-1:]*t, dim=0).flatten(-2).permute(0, 2, 1)   # [B*T, H*W, C]   [g,g,g,g,g] MFg

        concat0 = self.layer1(concat0, concat1)     # 输入(MFi,MFg)，计算Attention即公式（7）（8）

        #View
        concat0 = concat0.view(-1, h, w, c).permute(0, 3, 1, 2).contiguous()    # 调整MFi^

        return concat0.chunk(t, dim=0)


def single_head_full_attention(q, k, v):    # 计算Attention
    # q, k, v: [B, L, C]
...
class AttnLayerNoV(nn.Module):
    def __init__(self, d_model=128, mid_dim=128, no_ffn=False, ffn_dim_expansion=2):
        ...
        if not self.no_ffn:
            ...
            self.mlp = nn.Sequential(   # 对应公式（8）中的MLP
    ...
    def forward(self, source, target,
        ...
        message = single_head_full_attention(query, key, value)  # [B, L, C] 对应公式（7）计算Attention

        message = self.merge(message)  # [B, L, C]
        message = self.norm1(message)   # 公式(8) AWO

        if not self.no_ffn:
            message = self.mlp(torch.cat([source, message], dim=-1))    # 公式(8)[MFi, AWO]
            message = self.norm2(message)   # LayerNorm

        return source + message     # 公式(8）MFi + MLP([MFi,AWO])
